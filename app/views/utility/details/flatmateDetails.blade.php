...
<section class="uk-container uk-container-center">
      ...
      <article class="uk-article uk-width-medium-8-10">
        <ul class="uk-breadcrumb">
            ...
            <li><a href="{{ URL::route('getFlatmatesSearchQueryResult')}}">Flatmates</a></li>
        ...
        <div class="uk-panel uk-panel-box uk-panel-box-secondary">
            <h2>Flatmates Item Details</h2>
            <ul class="uk-grid uk-text-justify uk-container-center uk-width-9-10" data-uk-grid-margin="">
                <li class="uk-width-medium-1-3"><h5 style="margin-bottom:0px;" class="uk-text-muted"><b>Price</b></h5><i class="uk-icon-rupee"></i> {{$product->monthly_rent}}/-</li>
                <li class="uk-width-medium-1-3 uk-grid-margin"><h5 class="uk-text-muted"><b>Location</b></h5>{{ $product->location }}</li>
                <li class="uk-width-medium-1-3"><h5 class="uk-text-muted"><b>Posted by</b></h5>{{ $product->profile->user_type }}</li>
                <li class="uk-width-medium-1-3"><h5 class="uk-text-muted"><b>Facilities Available</b></h5>{{$product->facilities_available}}</li>
                <li class="uk-width-medium-1-3 uk-grid-margin"><h5 class="uk-text-muted"><b>Category</b></h5>{{$product->category}}</li>
            </ul>
        </div>
...
<section class="uk-container  uk-margin-large-top uk-margin-large-bottom">
    <h2>Other Flatmates in this college</h2>
<div class="uk-grid tm-grid-truncate" data-uk-grid-margin="">
    @foreach($other_flatmates as $other_flatmate)
    <div class="uk-width-medium-1-3">
        <div class="uk-panel uk-panel-box uk-panel-box-secondary">
            <h3 class="uk-panel-title"><a href="{{ URL::route('getFlatmatesDetails',$other_flatmate->ad_title .'~'.$other_flatmate->id)}}">{{$other_flatmate->ad_title}}</a></h3>
            <p>Location: {{$other_flatmate->location}}</p>
            <em><i class="uk-icon-rupee"></i> {{$other_flatmate->monthly_rent}}/-</em>
            <a class="uk-align-right" href="{{ URL::route('getFlatmatesDetails',$other_flatmate->ad_title .'~'.$other_flatmate->id)}}">VIEW</a>
        </div>
    </div>
    @endforeach
</div>
