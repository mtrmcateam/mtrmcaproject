...
<section class="uk-container uk-container-center">
      ...
      <article class="uk-article uk-width-medium-8-10">
        <ul class="uk-breadcrumb">
            ...
            <li><a href="{{ URL::route('getCarPoolSearchQueryResult')}}">Car Pool</a></li>
        ...
        <div class="uk-panel uk-panel-box uk-panel-box-secondary">
            <h2>Car Pool Details</h2>
            <ul class="uk-grid uk-text-justify uk-container-center uk-width-9-10" data-uk-grid-margin="">
                <li class="uk-width-medium-1-3"><h5 class="uk-text-muted"><b>Price Per Passenger</b></h5><i class="uk-icon-rupee"></i> {{$product->selling_price}}/-</li>
                <li class="uk-width-medium-1-3 uk-grid-margin"><h5 class="uk-text-muted"><b>Departure Date</b></h5>{{ $product->departure_date }}/-</li>
                <li class="uk-width-medium-1-3"><h5 class="uk-text-muted"><b>Posted by</b></h5>{{ $product->profile->user_type }}</li>
                <li class="uk-width-medium-1-3"><h5 class="uk-text-muted"><b>Departure Time</b></h5>{{$product->departure_time}}</li>
                <li class="uk-width-medium-1-3 uk-grid-margin"><h5 class="uk-text-muted"><b>Pickup Location</b></h5>{{ $product->pickup_location }}</li>
                <li class="uk-width-medium-1-3 uk-grid-margin"><h5 class="uk-text-muted"><b>Destination</b></h5>{{ $product->destination_location }}</li>
                <li class="uk-width-medium-1-3 uk-grid-margin"><h5 class="uk-text-muted"><b>Mode of Transport</b></h5>{{$product->category}}</li>
            </ul>
        </div>
...
<section class="uk-container  uk-margin-large-top uk-margin-large-bottom">
    <h2>Other CarPool in this college</h2>
<div class="uk-grid tm-grid-truncate" data-uk-grid-margin="">
    @foreach($other_carpools as $other_carpool)
    <div class="uk-width-medium-1-3">
        <div class="uk-panel uk-panel-box uk-panel-box-secondary">
            <h3 class="uk-panel-title"><a href="{{ URL::route('getCarPoolDetails',$other_carpool->ad_title .'~'.$other_carpool->id)}}">{{$other_carpool->ad_title}}</a></h3>
            <p>Destination: {{$other_carpool->destination_location}}</p>
            <em><i class="uk-icon-rupee"></i> {{$other_carpool->selling_price}}/-</em>
            <a class="uk-align-right" href="{{ URL::route('getCarPoolDetails',$other_carpool->ad_title .'~'.$other_carpool->id)}}">VIEW</a>
        </div>
    </div>
    @endforeach
</div>
