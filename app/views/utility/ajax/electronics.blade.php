    ...
    <section class="uk-panel uk-panel-box-secondary" data-uk-alert>
            <div class="uk-grid">
                ...
                <div class="uk-width-medium-7-10">
                    <h2><a style="color:#1ABC9C;" href="{{ URL::route('getElectronicsDetails',$product->ad_title .'~'.$product->id)}}" target="blank">{{ $product->ad_title }}</a><div class="uk-align-right" style=""><small><a href="#" class="uk-alert-close uk-close"></a></small></div></h2>
                    ...
                    <div class="uk-grid uk-margin-top uk-grid-small">
                        <div class="uk-width-1-3"><h5 class="uk-text-muted"><b>Brand Name</b></h5>{{ $product->brand_name }}</div>
                        <div class="uk-width-1-3"><h5 class="uk-text-muted"><b>Condition</b></h5>{{ $product->item_condition }}</div>
                        <div class="uk-width-1-3"><h5 class="uk-text-muted"><b>Category</b></h5>{{ $product->category }}</div>
                    </div>
                    <div class="uk-margin-top">
                    <b class="uk-text-muted">Available in</b> <a href="{{ URL::route('getElectronicsKeywordSearchResult',array('college_id',$product->profile->college_id))}}">{{ $product->profile->college_id }}</a>
                    <a class="uk-align-right uk-button uk-button-primary" href="{{ URL::route('getElectronicsDetails',$product->ad_title .'~'.$product->id)}}" target="blank" >View More</a>
